...
def inject_current_year():
    """ Injeta o ano atual no contexto de todos os templates. """
...
# --- Rotas de Autenticação e Páginas Estáticas ---
...
def login():
    ...
    if request.method == 'POST':
        # Alterado para buscar o usuário pelo email em vez do username
        user = User.query.filter_by(email=request.form.get('email')).first()
        if user and user.check_password(request.form.get('password')):
            login_user(user, remember=True)
            return redirect(url_for('dashboard'))
        # Mensagem de flash atualizada para refletir a mudança
...
def create_party():
    ...
    party_code = generate_unique_code(Party, 'party_code', 6)
    share_code = generate_unique_code(Party, 'share_code', 8)
...
def public_scanner():
    if request.method == 'POST':
        ...
        if party:
            ...
            resp.set_cookie('party_code', party_code_input, max_age=30*24*60*60) # Cookie de 30 dias
            return resp
        else:
...
def create_payment():
    ...
    payload = {
        "amount": 10000,
        "expiresIn": 3600,
        "description": f"Upgrade VIP QRPass - {current_user.username}",
        "customer": {
    ...
    try:
        ...
        pix_qr_code = response_data.get('brCodeBase64')
        pix_emv = response_data.get('brCode')
        pix_transaction_id = response_data.get('id')
        
        if not all([pix_qr_code, pix_emv, pix_transaction_id]):
            raise ValueError("Resposta da API de PIX incompleta.")
            
        current_user.payment_charge_id = pix_transaction_id
        ...
        app.logger.error(f"Erro de comunicação: {e}")
        flash('Não foi possível gerar a cobrança PIX. Tente novamente.', 'danger')
        return redirect(url_for('upgrade'))
    except (ValueError, KeyError) as e:
        app.logger.error(f"Erro ao processar resposta da API: {e}")
...
def check_payment_status(pix_id):
    if current_user.payment_charge_id != pix_id:
...
def abacatepay_webhook():
    ...
    if event_type == 'pix_qr_code.paid' and transaction_id:
        ...
        if user:
            ...
            app.logger.warning(f"Usuário não encontrado para o PIX ID do webhook: {transaction_id}")
...
def get_stats(party_id):
    party = db.session.get(Party, party_id) or abort(404)
    stats = get_party_stats_data(party_id)
...
def handle_guests(party_id):
    ...
    if request.method == 'POST':
        ...
        if not name:
            return jsonify({'error': 'O nome do convidado é obrigatório.'}), 400
        ...
        if not qr_image_filename:
            return jsonify({'error': 'Falha ao gerar a imagem do QR Code.'}), 500
    ...
    return jsonify({
        'guests': [{
            'id': g.id, 'name': g.name, 'qr_hash': g.qr_hash, 'entered': g.entered, 
            'qr_image_url': g.qr_image_url, 'check_in_time': g.get_check_in_time_str(), 
            'added_by': g.adder.username
...
def mark_entered(party_id, qr_hash):
    ...
    if not guest:
        return jsonify({'error': 'QR Code inválido para esta festa'}), 404
        
    current_time_brasilia = datetime.now(BRASILIA_TZ)
    
    if guest.entered:
        message = f'{guest.name} já entrou às {guest.get_check_in_time_str()}.'
        is_new_entry = False
    else:
        guest.entered = True
        ...
        db.session.commit()
        is_new_entry = True
        message = f'Entrada liberada! Bem-vindo(a), {guest.name}!'
        
    return jsonify({
        'id': guest.id, 'name': guest.name, 'qr_hash': guest.qr_hash, 
        'entered': guest.entered, 'message': message, 
        'is_new_entry': is_new_entry, 'check_in_time': guest.get_check_in_time_str()
    })
...
def edit_guest_name(party_id, qr_hash):
    ...
    if Guest.query.filter(Guest.name == new_name, Guest.party_id == party_id, Guest.qr_hash != qr_hash).first():
        return jsonify({'error': f'Já existe um convidado com o nome "{new_name}" nesta festa.'}), 409
...
def toggle_entry_manually(party_id, qr_hash):
    ...
    if guest.entered:
        guest.check_in_time = current_time_brasilia
        action = f"marcado(a) como PRESENTE"
    else:
        guest.check_in_time = None
        action = "marcado(a) como AUSENTE"
...
class PDF(FPDF):
    ...
    def header(self):
        ...
        title = f'Relatório do Evento: {self.party_name}'
    ...
    def draw_stats_summary(self, stats_data):
        ...
        stats_items = [("Convidados:", str(stats_data['total_invited'])), ("Entraram:", str(stats_data['entered_count'])), 
                       ("Não Entraram:", str(stats_data['not_entered_count'])), ("Comparecimento:", f"{stats_data['percentage_entered']:.2f}%")]
    ...
    def chapter_body(self, guests_data_table):
        ...
        for i, guest_obj in enumerate(guests_data_table):
            ...
            self.cell(col_widths["check_in"], 7, guest_obj.get_check_in_time_str(), 1, 1, 'C', fill)
...
def export_guests_csv(party_id):
    ...
    for guest_obj in guests_data:
        cw.writerow([guest_obj.name, 'Sim' if guest_obj.entered else 'Não', guest_obj.get_check_in_time_str()])
        
    return Response(
        si.getvalue(),
        mimetype="text/csv",
        headers={"Content-disposition": f"attachment; filename=lista_{party.name.replace(' ', '_')}.csv"}
    )
...
def export_guests_pdf(party_id):
    ...
    guests_for_table = get_all_guests_for_export(party_id)
    stats_for_chart = get_party_stats_data(party_id)
